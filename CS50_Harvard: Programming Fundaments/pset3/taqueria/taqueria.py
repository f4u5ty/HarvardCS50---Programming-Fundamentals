...
and so the user’s cursor (and subsequent prompt) might thus remain on the same line as your program’s own prompt.
...
def place_order():
    menu = {				#creation of menu dictionary
    "Baja Taco": 4.25,
    "Burrito": 7.50,
    "Bowl": 8.50,
    "Nachos": 11.00,
    ...
    while True:
        try:
            ...
            order_keys = orders.append(order)
            total_cost += menu[order]
            print("Total: ${0:2.2f}".format(total_cost))


        except EOFError:
            #print("Inside EOFError")

            print("\nTotal: ${0:2.2f}".format(total_cost))
            return total_cost
