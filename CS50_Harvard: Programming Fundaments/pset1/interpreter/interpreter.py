...
def interpreter():
    expsn = input("Expression: ")
    expsn = expsn.strip()                             #Strips white space on the ends.
    expsn = expsn.split(" ")                          #Splits by assumed white space.
    #print(expsn)                                     #Print statement for debugging.
    x = float(expsn[0])
    y = expsn[1]
    #print(y)
    z = float(expsn[2])
