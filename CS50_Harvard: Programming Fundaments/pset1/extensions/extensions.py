...
def file_type():
    type = input("Please enter a filename: ")
    type = type.strip().lower()                         #clean the input
    #print("type.find('.'): ", str(type.find('.')) )    #print state that shows what find method returns. (debug)
    type = type[type.find('.')+1:]                      #change type var from entire string, to everything after the period.
                                                        #string is being slice line above to include everything after the period.
    while "." in type:
        if type.find(".") > 0:                          #loop which keeps checking for periods until there are none. 
            type = type[type.find('.')+1:]

    #print(type)                                        #de-bugging print statement.
    match type:                                         #use match-case to output a type.
        case "gif" :
            print("image/gif")
